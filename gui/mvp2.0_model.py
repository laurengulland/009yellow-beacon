...
class Data_to_Display(object): #object to be passed back in response to data_within_range().
	...
	def add_scout_point(self,scout_id,location,is_current_position=False):
		...
		if is_current_position:
			self.current_positions[scout_id] = location
		if scout_id in positions_list:
			self.positions_list[scout_id] = positions_list[scout_id].append(location)
		else:
			self.positions_list[scout_id] = [location]
	...
	def add_waypoint(self,waypoint_id,location,label,poi_type,scout_id):
		...
		self.waypoint_positions[waypoint_id] = location
